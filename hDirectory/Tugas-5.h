...
void orderHistory() {
    ...
    for(int i = 0; i<DrinkLen; i++){
        if(ArrayDrink[i].drink_cooking_time<=0){
			printf("| %-4d| %-21s| %-7d| -          |-           | %-11s| %c     | %-20s |", index, ArrayDrink[i].food_name, ArrayDrink[i].food_price, ArrayDrink[i].drink_flavor, ArrayDrink[i].drink_size, ArrayDrink[i].created_at);
			index++;
		}
	}
    puts("");
